...
class CarController(CarControllerBase, GasInterceptorCarController):
  def __init__(self, dbc_names, CP, CP_SP):
    ...
    self.rsa_syncid = 0  # RSA cluster-sign rolling counter (1..15)
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    # RSA cluster speed-limit sign (experimental): drive the dash sign from the resolved
    # (camera + OSM) limit at ~1Hz. The panda blocks the camera's RSA copies while this runs
    # (toyota.h fwd hook); the resolved value already applies the driver's signs-first policy.
    if self.CP_SP.flags & ToyotaFlagsSP.RSA_CLUSTER and self.frame % 100 == 0:
      self.rsa_syncid = self.rsa_syncid % 15 + 1  # 1..15 rolling, wraps 15->1
      speed_limit_mph = round(CC_SP.speedLimitResolved * CV.MS_TO_MPH) if CC_SP.speedLimitResolved > 0 else 0
      if 0 < speed_limit_mph <= 120:
        can_sends.extend(toyotacan.create_rsa_commands(speed_limit_mph, self.rsa_syncid))
      else:
        can_sends.extend(toyotacan.create_rsa_commands(0, self.rsa_syncid))  # blank the sign
